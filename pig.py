class Game:
    ...
    def start_game(self):
        """
        Determines who goes first. If human rolls a value that is tied or higher than computer, human goes first. Otherwise, computer goes first.
        """

        if value self.human_roll() >= to self.computer_roll():
            self.human_roll() == first_player
        else:
             self.computer_roll() == first_player
        return first_player
    ...
    def play_round(self):
        ...
        if first_player == self.human_roll:
            self.human_roll()
            self.print_human_roll() 
            self.ask_human_to_roll
            self.add_roll_to_human_roll
        elif first_player == self.computer_roll:
            self.computer_roll
            self.print_computer_roll
